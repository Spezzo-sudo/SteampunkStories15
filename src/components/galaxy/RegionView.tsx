...
import {
  MissionComposer,
  type MissionDraft,
} from '@/components/galaxy/MissionComposer';
import {
  MissionTimeline,
  type MissionReport,
} from '@/components/galaxy/MissionTimeline';
...
const RegionViewComponent: React.FC<RegionViewProps> = ({ region }) => {
  ...
  const logisticNodes = useMemo(() => tiles.filter((tile) => tile.settleable).slice(0, 3), [tiles]);

  const missionReports = useMemo<MissionReport[]>(() => {
    const [primary, secondary, tertiary] = logisticNodes;
    const regionLabel = `Region (${region.RQ}, ${region.RR})`;
    const labelFromTile = (tile?: TileData | null) =>
      tile ? `Sektor (${tile.q}, ${tile.r})` : regionLabel;
    return [
      {
        id: `${region.regionId}-strike`,
        action: 'attack',
        status: 'success',
        originLabel: primary ? `Bastion ${primary.q}/${primary.r}` : regionLabel,
        targetLabel: secondary ? `Vorposten ${secondary.q}/${secondary.r}` : labelFromTile(),
        eta: 'abgeschlossen',
        summary: 'Stoßtruppe hat eine Bandenschanze ausgeschaltet und den Versorgungskorridor geöffnet.',
        highlights: [
          'Gefechtsdauer: 3 Runden, Verlustquote 8 %',
          'Artillerie zerlegte die Dampfpanzersperre nach 90 Sekunden',
          'Beute: zwei Ätherkerne, Vorräte für 6 Tage',
        ],
      },
      {
        id: `${region.regionId}-colonise`,
        action: 'colonise',
        status: 'in-progress',
        originLabel: primary ? `Koloniekonvoi ab ${primary.q}/${primary.r}` : regionLabel,
        targetLabel: tertiary ? `Gewähltes Tal ${tertiary.q}/${tertiary.r}` : labelFromTile(),
        eta: 'noch 12 h',
        summary: 'Kolonisten errichten Schotten, richten Werkstätten ein und erwarten Verstärkung.',
        highlights: [
          'Schildgenerator zu 68 % geladen',
          'Benötigt weitere 30 Tonnen Baustahl für Bastionsturm',
          'Pfad intakt – Eskorte meldet keine Feindkontakte',
        ],
      },
      {
        id: `${region.regionId}-trade`,
        action: 'trade',
        status: 'success',
        originLabel: secondary ? `Handelsfeste ${secondary.q}/${secondary.r}` : labelFromTile(primary),
        targetLabel: tertiary ? `Partnergilde ${tertiary.q}/${tertiary.r}` : labelFromTile(),
        eta: 'vor 4 h',
        summary: 'Karawane kehrte mit seltenen Legierungen und Reparaturkits zurück. Verbündete Bande stärkt Ruf.',
        highlights: [
          'Tausch: 120 Schocklanzen gegen 3 Speicherkerne',
          'Begleitende Sanitäter behandelten 14 Verwundete entlang der Route',
          'Einladung zur gemeinsamen Offensive gegen die Rauchbruderschaft',
        ],
      },
    ];
  }, [logisticNodes, region.RQ, region.RR, region.regionId]);
  ...
  const handleCreateMission = useCallback((draft: MissionDraft) => {
    setMissionDrafts((previous) => [draft, ...previous].slice(0, 5));
    setBanner({ tone: 'success', message: 'Einsatz gespeichert – siehe Kampflog.' });
  }, []);
  ...
  return (
    <div className="flex h-full flex-col gap-4 p-4 md:flex-row">
      ...
      <aside className="flex h-full min-h-[260px] flex-1 flex-col gap-3 md:max-w-md">
        ...
        />
        <MissionTimeline drafts={missionDrafts} reports={missionReports} />
        <RegionTileTable
          tiles={filteredTiles}
